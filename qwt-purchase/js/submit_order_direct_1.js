...
$(function () {
    ...
    var direct_buy_good_id=window.sessionStorage.direct_buy_good_id;
    var direct_buy_num=window.sessionStorage.direct_buy_num;
    var direct_buy_price=window.sessionStorage.direct_buy_price;
    console.log(direct_buy_price);
...
//查看用户身份
    ...
    $(".choose_bank").on('change',function () {
        if($(this).val() == "其他银行"){
            $("#other_bank").css('display','block');
            $('#other_bank').blur(function () {
                var reg= /^[\u4e00-\u9fa5]+$/;// 全汉字
                if($(this).val()==""){
                    $(".info_warn").eq(4).html('<div class="re_warn">银行名称不能为空</div>');
                }
                else if(!reg.test($(this).val())){
                    $(".info_warn").eq(4).html('<div class="re_warn">银行名称必须全为汉字</div>');
                }
                else {
                    $(".info_warn").eq(4).html('<div class="re_warn">请填写银行账号</div>');
                }
            })
        }else{
            $("#other_bank").css('display','none');
            $("#other_bank").val("");
            $("#bank_account").val("");
    ...
    $("#submit_order").click(function () {
        if(receiver_flag){
            ...
            receiver_name = $("input:radio[name='address']:checked").attr("receiver");
            detail_address = $("input:radio[name='address']:checked").attr("detailAD");
            address =$("input:radio[name='address']:checked").attr("address")+" "+$("input:radio[name='address']:checked").attr("address1")+" "+$("input:radio[name='address']:checked").attr("address2");
            phone = $("input:radio[name='address']:checked").attr("phone");
            console.log(address)
            ...
            if (receipt_errors == 0) {
                $.ajax({
                    url: http + "order/direct?uid=" + u_id + "&sid=" + s_id+"&commodityId="+direct_buy_good_id+"&number="+direct_buy_num,
                    xhrFields:{withCredentials: true},
                    data: JSON.stringify({
                        "invoice": {
                    ...
                    success: function (data) {
                        ...
                        if (data.msg == "SUCCESS") {
                            ...
                            if (payWay_num == 1 || payWay_num == 2 || payWay_num == 3) {
                                ...
                                tempwindow.location="submit_pay.html";
                            }
    ...
    $("#new_ad").click(function () {
        ...
        $("#new_address_Modal").off('hide.bs.modal').on('hide.bs.modal', function () {
    ...
    $(".name_detail").focus(function () {
        ...
        $(this).parent().append('<span class="tips">请填写您的真实姓名,长度不超过20</span>');
    });
...
//   获得购物订单中的物品  根据上一个页面传来的商品id和数量

    $.ajax({
        url:http+"commodity/"+direct_buy_good_id+"?uid="+u_id+"&sid="+s_id,
        type:"get",
        xhrFields:{withCredentials: true},
        success:function (data) {
            console.log(data);
            if(data.msg=="SUCCESS"){
                var buy_list=data.data;
                var want_buy_good ='<div class="list">'+'<div class="pic">'+"<img src="+buy_list.logoPath+" alt=''>"+ '</div>'+
                    '<span class="good">'+buy_list.commodityName+buy_list.standardName+ '</span>'+
                    '<span class="price">'+buy_list.unitPrice+'</span>'+
                    '<span class="num">'+direct_buy_num+'</span>'+'<span class="total">'+
                    +(buy_list.unitPrice*direct_buy_num).toFixed(2)+'</span>'+'</div>'
                $(".lists").append(want_buy_good);
            //获得总金额
                $(".total_price_num").html(direct_buy_price);
...
//点击提交订单
    $("#submit_order").click(function () {
        ...
        // if(pay_way==null||order_total_price==null){
        //     alert("数据错误！")
        // }
        // else{

        //     window.sessionStorage.obj_total_price=order_total_price;
        //     window.sessionStorage.obj_pay_way=pay_way;
        // }
    })
    //点击取消订单
