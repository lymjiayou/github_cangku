...
$(function () {
    ...
    $.ajax({
        url:http+"order/commodity/bought?page=1&pageSize=16",
        type:"get",
        xhrFields:{withCredentials: true},
        success:function (data) {
            console.log(data)
            if(data.msg=="SUCCESS"){
                var my_all_goods=data.data.list;
                console.log(my_all_goods);
                // my_all_goods.length=0
                if(my_all_goods.length>0){
                    ...
                    for(var i=0;i<my_all_goods.length;i++){
                        var my_good='<li class="goods_info">'+'<div class="row">'+'<div class="col4">'+'<div class="thumbnail">'+
                            '<div class="pic_box" self_id="i_'+my_all_goods[i].commodity.id+'" commodityCode='+my_all_goods[i].commodity.commodityCode+'>'+'<img src='+my_all_goods[i].commodity.logoPath+' alt="">'+'</div>'+
                            '<div class="caption">'+'<h3>'+my_all_goods[i].commodity.commodityName+my_all_goods[i].commodity.standardName+'</h3>'+'<div class="goods_price">'+
                            '<i>'+'¥'+'</i>'+'<span class="price">'+my_all_goods[i].commodity.unitPrice+'</span>'+'<div class="add_car">'+
                            '<button class="btn btn-default" good_id=i_'+my_all_goods[i].commodity.id+'>'+'购物车'+'</button>'+'</div>'+'</div>'+'</div>'+'</div>'+'</div>'+'</div>'+'</li>';
                        $(".goods_list").append(my_good);
...
//获取点击进入的商品id
    ...
    $("#search_name").click(function () {
        ...
        $.ajax({
            url:http+"order/commodity/bought?page=1&pageSize=16&keywords="+good_name,
            type:"get",
            xhrFields:{withCredentials: true},
            success:function (data) {
                console.log(data)
                if(data.msg=="SUCCESS"){
                    ...
                    $(".pic_box").on("click",function () {
                        ...
                        window.sessionStorage.good_commodityCode=$(this).attr("commodityCode");
                        var selfID=$(this).attr("self_id").split("_")[1];
                        var commodityCODE=$(this).attr("commodityCode").split("_")[1];
                        window.open("goods_detail.html?selfID="+selfID+"&commodityCode="+commodityCODE);
...
function change_page(current_page) {
    ...
    // var s_id=window.sessionStorage.s_id;
    $.ajax({
        url:http+"order/commodity/bought?page="+current_page+"&pageSize=16",
        type:"get",
        xhrFields:{withCredentials: true},
        success:function (data) {
            if(data.msg=="SUCCESS"){
...
function name_change_page(name_current_page,good_name){
    ...
    // var s_id=window.sessionStorage.s_id;
    $.ajax({
        url:http+"order/commodity/bought?page="+name_current_page+"&pageSize=16&keywords="+good_name,
        type:"get",
        xhrFields:{withCredentials: true},
        success:function (data) {
            console.log(data)
