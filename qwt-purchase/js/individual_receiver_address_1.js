    ...
    $.ajax({
        ...
        success:function (address_list) {
            ...
            if(address_list.msg=="SUCCESS"){
                ...
                if(address_list.data.length>0){

                    for (var i = 0; i < addr_data.length; i++) {
                        var default_sign_html="";
                        if(addr_data[i].defaultReceiver){
                            default_sign_html='<img src="../images/default_address.png" alt="">'+'默认地址';
                        }
                        var receiver = '<tr class="receiver_content">' + '<th class="receiver_name_info">' + addr_data[i].receiver + '</th>' +
                            ...
                            '<th class="receiver_tel_info">' + addr_data[i].phone + '</th>' + '<th class="receiver_default_info">' +default_sign_html+ '</th>' +
                            ...
                            $.ajax({
                                ...
                                success:function (data) {
                                    ...
                                    if (data.msg=="SUCCESS"){
                                        ...
                                        if(receiver_info.defaultReceiver){
                                            console.log(111)
                                            $(".default_addr").attr("checked",true);
                                        }else {
                                            $(".default_addr").attr("checked",false);
                                        }
                                        //省市区三级分类获取
...
//                                                     "detailAddress": re_street_detail
//                                                 }),
//                                                 success: function (data) {
//                                                     if (data.msg == "SUCCESS") {
//                                                         console.log(data);
...
//                             $(".delete_del").click(function () {
//                                 $.ajax({
//                                     type:"delete",                        beforeSend: function(request) {                            var cooinfo=getCookie("XSRF-TOKEN");                            console.log(cooinfo);                            request.setRequestHeader("X-XSRF-TOKEN", cooinfo);                        },
//                                     xhrFields:{withCredentials: true},
//                                     url: http + "receiver/" + id + "?uid=" + u_id + "&sid=" + s_id,
//                                     success: function (data) {
//                                         if (data.msg == "SUCCESS") {
...
//
//             } else {
//                 // var obj_data = JSON.parse(address_list);
