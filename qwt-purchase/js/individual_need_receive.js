...
$(function () {
    ...
    //获取所有待收货订单
    $.ajax({
        url:http+"order?uid="+u_id+"&sid="+s_id+"&page=1&pageSize=10&state=3",
        type:"get",
                ...
                if(all_order_list.length>0){
                    ...
                    $.each(all_order_list,function (index) {
                        ...
                        $.each(order_list_info,function (i) {
                            ...
                            order_state_text="待收货";
                            order_operation='<button type="button" class="btn btn-success confirm">'+'确认收货'+'</button>';
                            // console.log(orders)
                        })
                   ...
                   common_operation()
                    // //添加分页
                    // var page_tab='<div class="change_page">'+'<nav aria-label="Page navigation">'+'<ul class="pagination">'+'<li class="disabled">'+
                    //     '<a href="#" aria-label="Previous">'+'<span aria-hidden="true">'+'上一页'+'</span>'+'</a>'+'</li>'+
                    //     '<li class="active">'+'<a href="#">'+'1'+'</a>'+'</li>'+'<li class="disabled">'+'<a href="#" aria-label="Next">'+
                    //     '<span aria-hidden="true">'+'下一页'+'</span>'+'</a>'+'</li>'+'</ul>'+' </nav>'+'</div>'+''+''+''+''+''+''+''+''+''+''
                    //
                    // $("#w_pay").append(page_tab);
                }else {
    ...
    $("#search_time").click(function () {
        ...
        $.ajax({
            url:http+"order?uid="+u_id+"&page=1&pageSize=10&sid="+s_id+"&startTime="+start_time+"&endTime="+end_time+"&state=3",
            type:'get',
            xhrFields:{withCredentials: true},
...
function change_page(current_page) {
    ...
    //获取所有待收货订单
    $.ajax({
        url:http+"order?uid="+u_id+"&sid="+s_id+"&page="+current_page+"&pageSize=10&state=3",
        type:"get",
        xhrFields:{withCredentials: true},
        success:function (order_list) {
            if(order_list.msg=="SUCCESS"){
...
function change_time_page(time_current_page,start_time,end_time) {
    ...
    $.ajax({
        url:http+"order?uid="+u_id+"&page="+time_current_page+"&pageSize=10&sid="+s_id+"&startTime="+start_time+"&endTime="+end_time+"&state=3",
        type:"get",
        xhrFields:{withCredentials: true},
        success:function (order_list) {
            if(order_list.msg=="SUCCESS"){
    ...
    $(".confirm").each(function (index,ele) {
        $(ele).on("click",function () {
            ...
            $(".confirm_take_order").click(function () {
                $.ajax({
                    url:http+"order/receipt/"+order_list_id+"?uid="+u_id+"&sid="+s_id,
                    type:"patch",
