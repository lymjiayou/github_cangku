...
$(function () {
    ...
    //获取所有待评价订单
    $.ajax({
        url:http+"order?uid="+u_id+"&sid="+s_id+"&page=1&pageSize=10&state=4",
        type:"get",
                ...
                if(all_order_list.length>0){
                    ...
                    $.each(all_order_list,function (index) {
                        ...
                        $.each(order_list_info,function (i) {
                            var sum=(order_list_info[i].commodityNumber*order_list_info[i].realPrice).toFixed(2);
                            ...
                            order_state_text="待评价";
                            order_operation='<button type="button" class="btn btn-info commit">'+'立即评价'+'</button>';
                            // console.log(orders)
                        })
    ...
    $("#search_time").click(function () {
        ...
        $.ajax({
            url:http+"order?uid="+u_id+"&page=1&pageSize=10&sid="+s_id+"&startTime="+start_time+"&endTime="+end_time+"&state=4",
            type:'get',
            xhrFields:{withCredentials: true},
...
function change_page(current_page) {
    ...
    var on_account=""
    $.ajax({
        url:http+"order?uid="+u_id+"&sid="+s_id+"&page="+current_page+"&pageSize=10&state=4",
        type:"get",
        xhrFields:{withCredentials: true},
        success:function (order_list) {
            if(order_list.msg=="SUCCESS"){
...
function change_time_page(time_current_page,start_time,end_time) {
    ...
    $.ajax({
        url:http+"order?uid="+u_id+"&page="+time_current_page+"&pageSize=10&sid="+s_id+"&startTime="+start_time+"&endTime="+end_time+"&state=4",
        type:"get",
        xhrFields:{withCredentials: true},
        success:function (order_list) {
            if(order_list.msg=="SUCCESS"){
