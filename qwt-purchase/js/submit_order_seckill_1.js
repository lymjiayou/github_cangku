...
$(function () {
    ...
    var seckill_buy_good_id=window.sessionStorage.seckill_buy_good_id;
    var seckill_buy_num=window.sessionStorage.seckill_buy_num;
    var seckill_buy_price=window.sessionStorage.seckill_buy_price;
    console.log(seckill_buy_price);
...
//查看用户身份
    ...
    $("#submit_order").click(function () {
        if(receiver_flag){
            ...
            if (receipt_errors == 0) {
                $.ajax({
                    url: http + "order/direct/seckill?uid=" + u_id + "&sid=" + s_id+"&commodityId="+seckill_buy_good_id+"&number="+seckill_buy_num,
                    xhrFields:{withCredentials: true},
                    data: JSON.stringify({
                        "invoice": {
...
//   获得购物订单中的物品  根据上一个页面传来的商品id和数量
    ...
    $.ajax({
        // url:http+"commodity/"+seckill_buy_good_id+"?uid="+u_id+"&sid="+s_id,
        url:http+"commodity/seckill/alone?id="+seckill_buy_good_id,
        type:"get",
        xhrFields:{withCredentials: true},
        success:function (data) {
            console.log(data);
            if(data.msg=="SUCCESS"){
                var buy_list=data.data.commodity;
                var want_buy_good ='<div class="list">'+'<div class="pic">'+"<img src="+buy_list.logoPath+" alt=''>"+ '</div>'+
                    ...
                    '<span class="price">'+data.data.seckillPrice +'</span>'+
                    '<span class="num">'+seckill_buy_num+'</span>'+'<span class="total">'+
                    +(data.data.seckillPrice *seckill_buy_num).toFixed(2)+'</span>'+'</div>'
                $(".lists").append(want_buy_good);
            //获得总金额
                $(".total_price_num").html(seckill_buy_price);
